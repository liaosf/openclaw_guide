...
如果说 Gateway 是身体，Agent 是大脑，那么 **Skills** 就是让 OpenClaw 与世界交互的“机械臂”。本章将教你如何让 OpenClaw 具备超能力，并让它自动为你工作。

## 1. 技能 (Skills) 与 MCP

OpenClaw 的技能系统兼容 **MCP (Model Context Protocol)** 标准。简单来说，任何遵循该标准的 Node.js 脚本目录都可以被 OpenClaw 加载为一项技能。
...
2.  **本地开发 (Local)**
    将你自己编写的 Skill 放入 `~/.openclaw/skills/` 目录。
    *结构示例*：
    ```
    ~/.openclaw/skills/
      └── my-custom-skill/
...
3.  **工作区专用 (Workspace)**
    放在 `<workspace>/skills/` 下的技能仅对该 Agent 可见。
...
在 `openclaw.json` 中，你可以为特定技能配置环境变量（如 API Key）：
...
OpenClaw 内置了一个强大的调度器，你不再需要 Linux 的 `crontab`。
...
除了定时触发，OpenClaw 还能响应外部事件（如 GitHub Push, IFTTT, Zapier）。
...
**场景实战**：
你可以结合 **IFTTT** 监控 Gmail 邮件，收到老板邮件后调用 Webhook，让 OpenClaw 总结邮件内容并推送到你的 Telegram。

**下一步**：我们将探讨如何将 OpenClaw 部署在服务器上，并通过 Tailscale 实现远程访问。
